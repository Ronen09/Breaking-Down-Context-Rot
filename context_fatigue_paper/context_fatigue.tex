...
% TODO before submission: create the anonymized mirror at https://anonymous.4open.science
% and paste its URL over the XXXX placeholder.
\makeatletter
\if@anonymous
  \newcommand{\repourl}{https://anonymous.4open.science/r/Breaking-Down-Context-Rot-XXXX/}
\else
...
smaller, more distant, more contested fraction of it). We test the setting where the two come
apart: streams of \emph{localized tasks}, each answerable from its own question, so that
accumulation grows the context without diluting anything the current answer needs. In that
stream, context length, evidence distance, context confusability, and demonstrated answer shape
vary independently, and a span-attention clamp \emph{sets} any span's post-softmax attention
share. Accumulation alone shows no net cost: accuracy and instruction adherence stay flat to
$90\%$ fill, with bounded nulls. Moving a task's evidence back into the context turns it into
\emph{retrieval}, and this displacement costs accuracy, with attention mass \emph{causally
sufficient} for the cost. Removing
the evidence's share with nothing else moved reproduces the full penalty, smoothly and without a
threshold. Near-duplicate context costs further accuracy while the evidence's attention barely
moves. Closing every context mention of the answer options recovers $56\%$ of that penalty
against a random-closure control at zero, so competition is attention-mediated too, through the
\emph{competitors} rather than the evidence. Filler that demonstrates an \emph{applicable} answer
shape erodes an instructed output format completely ($0.875\to0.000$ within three filler turns,
accuracy unharmed) while the instruction's attention holds and its content stays linearly
decodable. Attention still carries causal \emph{weight} there: clamping the instruction's share
down collapses compliance with nothing competing, clamping it up restores compliance against
$42$ counter-exemplars, and restating the instruction restores it without surgery. The closure
that rescues competition restores nothing here. The demonstrated mode is set at prefill, and
counterfactual patching localizes its carrier to the chat template's delimiter tokens in the
lower half of the stack. In localized streams, context rot is not a decay of competence with
length. It is displacement, competition, and precedent, and attention dilution proper is only
the first.
\end{abstract}
...
The tempting reading is that length wears the model down. But long context comes in two shapes,
and they are not the same problem. In the first, \emph{one task spans the whole context}: a
long document, a buried needle, a growing codebase. Length there mechanically \emph{dilutes}
the answer-bearing tokens among irrelevant ones, so a measured degradation conflates the cost
of accumulation with the cost of the target being harder to attend to (``lost in the middle,''
\citealp{liu2023lost}). In the second, the context is a stream of \emph{localized tasks}, each
...
its two components, the evidence becoming \emph{distant} and the context becoming
\emph{confusable}, and test each causally. \textbf{RQ3}: if adherence survives context fill,
...
\begin{itemize}
  ...
  \item \textbf{Competition} is a \emph{second} mechanism, costing accuracy at
        constant \emph{evidence} mass. Closing the competitor mentions recovers $56\%$ of
        the penalty, so both accuracy mechanisms are attentional but on different spans, and
...
which always sits at the end of the context. Accumulation therefore grows the context without diluting the information the current answer
...
we can move the evidence $k$ user turns back (\emph{displacement}, which rebuilds the retrieval
...
so it separates nothing. Aggregate raw-share metrics such as the goal-accessibility ratio of
\citet{dongre2026attentioncloses} inherit this arithmetic decline by construction.
...
${\sim}10^{-4}$, and scale $0$ removes the span from attention entirely. We call this \emph{closing} the span. Closure is the hard-masking intervention of
\citet{dongre2026attentioncloses}, recovered as the clamp's endpoint, so their ablation and our
graded doses sit on one dial, and the clamp is bidirectional, so mediation can be tested by
...
what a decodable direction \emph{does}, we steer with mean-difference vectors at decode time
...
Appendix~\ref{app:qwen} replicates the program on Qwen2.5-7B-Instruct. Every mechanism experiment runs to a $4$k window on both families, so no arm comparison is
...
order, so unshuffled gold letters are $71.4\%$ ``A'', Appendix~\ref{app:tasks}).
...
Accuracy is a fraction throughout and ``points'' are percentage points, the scale of the
joint-fit coefficients (Appendix~\ref{app:stats}). Per-condition $n$ is bounded by the context window, so every clamp experiment is paired over
...
($\mathrm{corr}(\text{correct},\text{fill})=+0.11$ Instruct, $+0.07$ DPO). In-context learning on a homogeneous task could mask a hidden decay, so a
\emph{random-subject} stream draws each question uniformly from all 57 MMLU subjects, leaving
only the answer \emph{format} learnable from prior context. Accuracy is still flat: its correlation with fill is $-0.02$
(random, $n{=}728$) and $+0.032$ (coherent, $n{=}988$), computed over fill $<0.8$, and the
nulls are \emph{bounded}---at
$95\%$, hidden declines larger than $2.9$ points (coherent) or $5.5$ (random) are excluded.
Canary instruction adherence stays at ceiling throughout the program's one adherence arm
(Qwen2.5-7B-Instruct, full $32$k window). That null is narrower than it looks: the canaries
were easy and the accumulated history is itself compliant, so it shows adherence surviving
\emph{compliant} history, and \S\ref{sec:precedent} shows it does not survive history that
demonstrates a competing format.
...
is then fully monotone ($0.524\to0.413\to0.397\to0.343\to0.338$). One qualification: the decline saturates by $k{\approx}10$, and the $k{=}10$ and $k{=}20$
arms are not distinguishable. So the same model, on the same items at the same fill, degrades only when the evidence is
displaced. The localized null is not a
...
layer-24-indexed, $0.017$ on Table~\ref{tab:mass}'s all-layer scale.

\paragraph{Mass restoration.}
The converse is only partial: clamping the displaced evidence's share back \emph{up} to its
\texttt{local} level recovers little of the penalty (Table~\ref{tab:mass}). The natural suspect is the instrument, since a uniform clamp cannot reconstruct \emph{which}
...
evidence's ${\sim}280$ tokens, so the remainder is either that within-span token pattern or a
...
appears as some context case's \emph{distractor} (Appendix~\ref{app:tasks}). Paired
...
accuracy of $0.464$ (both gaps n.s.), and \texttt{disjoint} is the \emph{longest} arm while \texttt{near\_dup} is the shortest, so
length biases against the result. The effect survives restriction to parsed responses ($+0.082$, significant) but is \emph{not}
monotone in confusability (\texttt{random} sits above \texttt{disjoint}, n.s.), so we claim
that near-duplicate context is costly, not that cost rises with overlap.
...
most-read \emph{content} tokens of the context body, the same number of tokens as the
mentions and $0.087$ of the row's mass to their $0.0076$, recovers
...
chat template's turn delimiters. Closing \emph{that} set is broadly destructive, but what it
...
competitor closure that recovers $56\%$ of OLMo's penalty does nothing there ($+0.019$
...
all, so we treat competition as architecture-indexed and rest its positive claims on OLMo
...
transcript, so the history \emph{demonstrates} an answer shape while the system prompt
\emph{instructs} one. Two channels, one decision. When they agree, as in
\S\ref{sec:accumulation}'s compliant history, nothing surfaces. This section makes them
disagree, and the instruction is what gives way \citep[cf.][]{anil2024manyshot}.
...
items) while accuracy is unchanged ($0.525\to0.467$, n.s.) and parsing holds at $0.967$. The clamp level is one accumulation passes on
...
(Appendix~\ref{app:precedent-detail}). The instruction governs behaviour only when the transcript is silent.


\paragraph{Erosion tracks applicability, not attention.}
If the contest is with in-context learning, what should decide it is how far the demonstrated
shape \emph{transfers} to the probe. Three filler streams vary exactly that: \texttt{code}
...
$0.900$ at fill $0.778$, the deepest interpreted point). \texttt{gsm8k} erodes late and partially ($0.600$ at fill $0.48$, replies
...
Accuracy on the same replies is unharmed everywhere ($0.475$ shallow, $0.485$--$0.675$ deep): the
...
At depth $42$ (natural compliance $0.000$, accuracy $0.650$), clamping the system span back to
its cold-start share restores compliance to $1.000$ at an accuracy cost of $0.650\to0.425$:
...
at accuracy $0.450$ and with no intervention on the model. Both levers together give compliance
...
size-matched random control at its own window ($n{=}40$, depth $42$). The split instead localizes what exemplar-answer reading is worth to \emph{accuracy}, entirely
prefill-borne: closing during prefill costs $0.175$ $[0.075,0.300]$, closing during decode
costs exactly nothing item-for-item, and closing both is identical to closing prefill alone. The privileged reading is consistent with a template already in place by
...
sharpest dissociation in the paper: the \emph{same} scale-$0$ closure that recovers $56\%$ of
...
(leave-one-out AUC $0.875$ at layer $22$) within cells matched on depth, fill, and filler. The
mode is installable by direction: adding the mean-difference vector at decode time to a
demonstration-free context destroys the instructed format ($0.000$ vs.\ $0.425$ under a
norm-matched random vector). But no
linear direction we tried \emph{erases} it, even though two fitted projections drive the
layer's linear code to chance (Appendix~\ref{app:precedent-detail}). The code is low-rank,
not redundant, and the behavioral decision does not consume the linearly decodable component.
We can decode the mode and we can install it. Neither shows what carries it.
...
crossed cell (glue positions, layers $7$--$13$ only) still carries $42\%$. The format state rides the transcript's \emph{skeleton}, tokens the chat template already
supplies, consistent with delimiter and low-information tokens serving as computation slots
(Appendix~\ref{app:related}), and it is where the OLMo attention row concentrates
(\S\ref{sec:competition}, Fig.~\ref{fig:token-heatmap}), whose closure disrupts the
demonstrated shape. Unlike the mass absorbed by attention
...
unrelated-fact control at $-0.011$ in both).
...
conversations are \emph{longer}, length works against the effect. Across OLMo-2's
...
\emph{wrong} answers ($0.86\to0.95$, $r=+0.69$): confidence in wrong answers rises ${\sim}10$
...
\item \textbf{Displacement}, the retrieval case, produces a large accuracy cost that is
  reproduced in full by removing the evidence's attention mass, graded in that mass, and positional in tokens. This is the one mechanism that is attention dilution proper, where attention-side steering and
  positional recalibration \citep{zhang2023pasta,hsieh2024foundinmiddle} act on the mediating
  variable.
...
\item \textbf{Precedent} erodes not accuracy but \emph{compliance}, in a contest the
  instruction loses with its attention intact. This is the benign, self-generated counterpart
  ...
  \citep{davidson2025taskrepresentation,hendel2023taskvectors,todd2024functionvectors}.
\end{itemize}
...
at once, which is why length-indexed benchmarks cannot separate them
...
\looseness=-1
Per-condition $n$ is bounded by the window, so every interval here depends on the paired
analysis of \S\ref{sec:design}. The accumulation result is a \emph{net}
null with asymmetric precision (2.9 points coherent, 5.5 random), and the random-subject control
removes subject-level predictability rather than format or protocol learning, so we claim no
decline surfaces, not that accumulation carries no latent cost. For displacement, the necessity
direction of the mediation is only partially established. Head-uniformity is excluded as the
clamp's limitation, but the clamp still spreads each head's restored mass uniformly over the
span's tokens, so a within-span token pattern and a non-attention positional channel are not
separated. The competition result is one significant step rather than a gradient. Mild overlap
is indistinguishable from none, the near-duplicate arm also changes the selection regime, and we
do not claim cost rises with confusability. Its closure rescue leaves ${\sim}40\%$ of the gap
unexplained (residual not distinguishable from zero). The hot-set closure removes only the
high-attention form of instrument slack, so a low-mass paraphrase channel is not excluded, and
its ranking uses the all-layer mean row, so a layer- or head-indexed ranking could target
differently. The instruction-canary null is a floor effect, and the WildChat partition is
observational with no uncertainty on the tertile contrast. Competition rests on one family (Appendix~\ref{app:qwen}), and the low-share compliance
dissociation is OLMo-specific (Qwen's arm shares straddle its own
threshold).

\looseness=-1
The precedent results use one instruction family, graded by a rule-based checker at $n{=}40$
per cell, and the filler streams co-vary in genre, structure, and reply length, so the
applicability ordering rests on the stream contrast together with the one-example and reversal
experiments. Window-split closure rules out attention to the exemplar answers as what sustains
or installs the mode, but only for that span family. The mode's carrier localization is still
confounded with positional periodicity (the random control is size-matched, not
periodicity-matched), its patch readout is teacher-forced with no generation-level corroboration
under closure, and the cross-family statement is about the channel only, with no magnitude
comparison. The mode also remains unlocalized as a direction, since no linear direction we tried
erases it. Every mechanism experiment runs to a $4$k window (\S\ref{sec:design}), which makes the arms
comparable and bounds the claims: we order the mechanisms at $4$k and do not measure how their weights move at the $128$k windows the phrase ``context rot'' usually
names, though displacement is defined by attention share rather than token count and so carries
a prediction at any length. The accuracy null holds for localized tasks, and length-dependent
single-task settings are out of scope by design.
...
and $3.1\times10^{-5}$ on a GQA architecture (Qwen-2), both in fp32. The capture applies the additive attention
...
run), $0$ in the mmlu erosion arm, and $7$ in the code arm, all at its deepest depth, so that
...
so the cross-family contrasts below, the competition divergence included, are not window
...
competitor closure that recovers $56\%$ of OLMo's penalty does nothing here ($+0.019$
...
Build two transcripts identical in filler and probe but differing in the system instruction,
an \texttt{ANSWER:} prefix format against a one-line JSON format. Capture per-position hidden
states from each. Then patch the recipient's prefill with the donor's states, and read out the
two instructed formats' teacher-forced log-probability contrast. The deep \texttt{code} cell
runs at depth 15 with the system span closed, in the A$\to$B direction. The full-transcript
patch is the stage-1 run at $n{=}39$, and the bisection cells are $n{=}23$ after one overflow
skip.

\paragraph{The estimand, and why it is not the obvious one.}
Write $\Delta\ell$ for that log-probability contrast. The estimand is the interaction contrast
$\Delta\Delta = \Delta\ell_{\mathrm{patch}} - \Delta\ell_{\mathrm{self}}$, a
difference-in-differences whose baseline is the \emph{self-patch}: the
recipient's own open-captured states pushed through the identical procedure. It is not
measured against the unpatched transcripts. The reason is that donor states are captured with
the system span open, while a pure run's states form closed. Preflight showed that an
unpatched baseline confounds that capture difference with donor identity, and the
unrelated-fact control moved as much as the counterfactual patch. Against the self-patch
baseline the same control is null. Identification then requires only additivity, that
capture-openness shifts both arms' contrasts equally, which sharing one capture-and-splice
procedure across arms is designed to make true.

\paragraph{Where the state lives.}
The full-transcript patch moves the contrast by $\Delta\Delta = -2.488$. Bisecting by position,
every \emph{content} subset is near null. The context's assistant turns carry $-0.287$, its
...
\emph{inside} the turns, by role and by recency, had to come back null. The state rides
...
replies across both patch arms. The estimand therefore lives in the teacher-forced contrast
...
does not depend on that. The bisection asks \emph{where} the state travels, not which way it
pushes.
