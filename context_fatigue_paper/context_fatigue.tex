...
Every mechanism experiment runs to a $4$k window (\S\ref{sec:design}). The choice is what makes
the arms comparable, since per-condition $n$ is bounded by the window and every interval here is
paired. It also bounds the claims. We separate the mechanisms and order them at $4$k, and we do
not measure how their relative weights move at the $128$k windows the phrase ``context rot'' is
usually applied to. Displacement is defined by attention share rather than token count, so it
carries a prediction at any length, but that prediction is untested here.
