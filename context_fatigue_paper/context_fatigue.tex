...
recovers $56\%$ of that penalty, against a size-matched random-closure control at zero, so
...
current answer needs. The context-rot literature measures the first shape almost exclusively,
which is precisely where the two costs cannot be separated. This paper measures the second,
where they can, and the two accounts predict opposite outcomes for it
(Appendix~\ref{app:related}). Retrieval is the case between them: a localized stream whose
current answer depends on evidence sitting back in the context rather than at the query. That
intermediate case is what \S\ref{sec:displacement} builds, by taking a localized stream and
moving the evidence back.
...
Accumulation is the design's null arm (RQ1). The mechanisms that cost something are
displacement, competition, and precedent. Our contributions are the harness that separates
them and one causal result for each:
...
\begin{itemize}
  \item A harness that makes accumulation and dilution independent, by accumulating individually
        ...
        (\S\ref{sec:method}).
  \item \textbf{Displacement} is causally sufficient: removing the evidence's attention mass with
...
Appendix~\ref{app:qwen} replicates the program on Qwen2.5-7B-Instruct. Every mechanism
experiment reported here runs to a $4$k window, on both families, so that no arm comparison
is confounded by window size. The exceptions are not mechanism results. The adherence arm
in \S\ref{sec:accumulation} runs to $32$k, and the WildChat measurements behind the filler
design run to $16$k and $32$k. A $4$k window
...
Checkable instruction adherence (the canary instructions) stays at ceiling throughout, in the
program's one adherence arm, which ran on Qwen2.5-7B-Instruct to a full
32k window.
That null is narrower than it looks: the canaries were easy and the accumulated history is
itself compliant, so it shows adherence surviving \emph{compliant} history.
...
most-read \emph{content} tokens of the context body ($11\times$ the mentions' mass) recovers
...
format state rides the transcript's \emph{skeleton}, into tokens the chat template already
supplies, which is consistent with a literature on delimiter and low-information tokens serving
as computation slots (Appendix~\ref{app:related}). It is also
where the OLMo attention row concentrates (\S\ref{sec:competition},
Fig.~\ref{fig:token-heatmap}), and closing it disrupts
...
analysis of \S\ref{sec:design}. The accumulation result is a \emph{net} null with asymmetric
precision (2.9 points coherent, 5.5 random), so we claim no decline surfaces, not that
accumulation carries no latent cost. The competition result is one significant step rather than
a gradient, its closure rescue leaves ${\sim}40\%$ of the gap unexplained, and it is the one
mechanism that does not replicate across families. The precedent results use one instruction
family at $n{=}40$ per cell, and the mode's carrier localization is still confounded with
positional periodicity and has no generation-level corroboration under closure. Every
mechanism experiment runs to a $4$k window (\S\ref{sec:design}), which is what makes the arms
comparable and also what bounds the claims. We order the mechanisms at $4$k and do not measure
how their weights move at the $128$k windows the phrase ``context rot'' usually names, though
displacement is defined by attention share rather than token count and so carries a prediction
at any length. The accuracy null holds for localized tasks, and length-dependent single-task
settings are out of scope by design. Appendix~\ref{app:limitations} gives these in full.
...
This expands the summary in \S\ref{sec:limitations}.
...
with asymmetric precision (2.9 points coherent, 5.5 random), and the random-subject control
...
\paragraph{Precedent and the mode.}
...
scope by design. Every mechanism experiment runs to a $4$k window, which bounds the ordering to
that regime (\S\ref{sec:limitations}).
